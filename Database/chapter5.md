...
1. 有效性
    - 多值依赖的有效性与属性集的范围有关
    - 只要在R(U)的任何一个关系r中,元祖在x和y上的值满足函数依赖定义,则函数依赖x-->y在任何属性及w(x,y含于w含于u)上成立。
2.
    - 若函数依赖x-->y在R(U)上成立,则对于任何y'含于y均有x-->y'成立
    - 多值依赖x-->-->y若在R(U)上成立,不能断言对于任何y'含于y有x-->-->y'

### 4NF
定义:  
关系模式R<U,F>属于1NF,躲过对于R的每个非平凡多值依赖x-->-->y(y不含于x),x都有候选码,则R属于4NF。  
如果R属于4NF,则R属于BCNF  
不允许有非平凡且非函数依赖的多值依赖

### 规范化
- 关系数据库的规范胡理论是数据逻辑设计的工具
- 一个关系只要其分量都是不可份的数据项,它就是规范化的关系,但这只是最基本的规范化
- 规范化程度可以有多个不同的级别
- 关系模式规范化的基本步骤
    ![](./image/GFH.jpg)
...
``` md
    - 不能说规范化程度越高的关系模式越好
    - 在设计数据库模式结构时,必须对现实世界的实际情况和用户需求作进一步分析
    ,确定一个合适的,能够反映现实世界的模式
    - 上面的规范化步骤可以在其中任何一步终止
```
